...
contract DAOTBA is TokenCallbackHandler, IERC1271, ITokenBoundAccount, Votes, IERC6909Votes {
    ...
    // dummy functions to simulate being an ERC721/ERC20...
    ...
    // store the address of the Governor contract or the zero address
    // when setted to governor address, executeCall is only allowed trough governance
    address public onlyGovernor;
    ...
    // every holder may call this function if onlyGovernor is the zero address
    ...
    // trustless communities should use the Governor proposal system instead.
    function executeCall(
        address to,
        uint256 value,
        bytes calldata data
    ) external payable returns (bytes memory result) {
        // next line of code does all the magic here.
        // when onlyGovernor is zero, executeCall has its expected behaviour
        // every token holder is able to call executeCall, but
        // when onlyGovernor is a governor address, only calls to governor are allowed
        // this way, executeCall will only work trough the governance... BOOM !!
        require(onlyGovernor == address(0) || to == onlyGovernor, "OnlyGovernor");
