...
contract DAOTBA is TokenCallbackHandler, IERC1271, ITokenBoundAccount, Votes, IERC1155Votes {
    uint256 private _nonce;
    receive() external payable {}
    
    constructor() EIP712("DAOTBA", "0.0.0") {}
    
    // dummy functions to simulate being an ERC721...
    ...
    function uri() public view returns (string memory) {
        ...
        return IERC1155MetadataURI(tokenContract).uri(tokenId);
    } 
    ...
    function totalSupply() external view returns (uint256) {
        ...
        return IERC1155Extended(tokenContract).totalSupply(tokenId);
    }
    ...
    function transferVotingUnits(address from, address to, uint256 amount) external {
        ...
        if (chainId != block.chainid || tokenContract != msg.sender) revert();
    ...
    function _getVotingUnits(address account) internal view virtual override returns (uint256) {
        ...
        return IERC1155(tokenContract).balanceOf(account, tokenId);
    }   
    ...
    function supportsInterface(bytes4 interfaceId) public view override(TokenCallbackHandler) returns (bool) {
        return (super.supportsInterface(interfaceId) ||
                  ...
                  interfaceId == type(IERC1155Votes).interfaceId
        );
    }
